...
def extract_dot_vex(vex_file_location: str,
	...
	with open(DEFAULT_TEMP_FILE_LOCATION + "___ThIsisATemPoRaRyFiLE___.json") as content:
		...
		if os.path.isdir(save_folder_location)== False:
...
def main():
    ...
    print("You are running it in the console, it have less feature than the converter_ui \n highly suggrest running converter_ui.python instead")
	...
	if mode == 1 then:
		...
		update_dot_vex(vex_file, save_dir, save_name, open)
